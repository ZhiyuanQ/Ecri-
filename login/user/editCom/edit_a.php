<?php
	...
	if($_SESSION['USER']['statut'] == 'admin' || $_SESSION['USER']['statut'] == 'chercheur') {
		...
		$arr_temp = array('redacteur'=>$redacteur,'dateModif'=>$dateModif);
		...
		if($pdo->update($arr)){
			...
			echo "<script>alert('Insertion échouée,Veuillez contacter l'administrateur, ou réessayez.');window.location.reload();</script>";
		}
    } else {
